...
This directory is the Worker half of control protocol `2026-09-08.v2`. It keeps persistent IDs and all authoritative monetary values as canonical integer E8 USD decimal strings at the JavaScript/D1 boundary. Usage events remain on their separately versioned `2026-09-06.v1` schema. D1 is authoritative; `CONFIG_CACHE` only receives rebuildable, non-sensitive model/pricing cache entries and is never trusted for admission.
...
`X-Sub2API-Bridge-Version: 2026-09-08.v2` and a Container identity header.
...
All IDs and `balance_e8_usd` values are canonical decimal strings. Legacy
microusd request/response fields are emitted or accepted only when the value is
exactly representable; they are never the D1 authority. Mutation requests
require an `operation_id`, reject unknown fields, and are replay-safe.
...
D1 migration `0008_e8_money_and_pricing.sql` converts existing canonical
microusd balances and ledger rows to E8 USD without floating-point arithmetic.
It also adds append-only, digest-checked pricing versions and rules. Admission
fails closed unless exactly one active snapshot prices the requested model;
the admitted version, digest, and exact/family rule pattern are then immutable
facts on the gateway request. The current E8 pricing work is a foundation for
reservation and settlement, not a claim that those later state machines are
already complete.
