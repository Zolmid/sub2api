...
The locked traditional deployment is unchanged. The original `snapshot-postgres`, `export`, and `plan` commands retain their `legacy-postgresql-to-d1-0008/v1` contract. The explicit `-0017` commands retain the additive `legacy-postgresql-to-d1-0017/v1` profile and canonical migrations `0001_initial.sql` through `0017_email_runtime.sql`. Use the explicit `-0018` commands below for the current canonical `legacy-postgresql-to-d1-0018/v1` profile and canonical migrations through `0018_auth_sessions.sql`.

## Mapping boundary

The 0018 profile copies or deterministically transforms every legacy field that has an equivalent canonical D1 authority:
...
All target-only runtime/audit/outbox tables introduced through 0018 are declared in the bundle as either `empty-before-import` or `trigger-managed-from-source-inserts`. The planner verifies that declared operational state is pristine before a first import. It explicitly initializes `users.balance_version=0`, `accounts.credential_version=1`, and `accounts.credential_fingerprint=NULL` through canonical column defaults. `auth_sessions`, `auth_session_family_revocations`, `auth_session_audit_events`, and `auth_session_rotation_witnesses` are target-only ephemeral refresh-session state derived from legacy Redis; they must start empty and no source session rows or raw tokens are invented or copied. An identical replay is permitted only after the bundle digest provenance has been accepted.

## Private artifact rules

Use a private directory outside the repository. Every input and output must have a unique absolute path. Do not put the PostgreSQL DSN or encryption keys in positional arguments, filenames, logs, or generated artifacts. New 0017 and 0018 outputs refuse to overwrite existing paths.
...
## 1. Capture and export the 0018 bundle

Run from `backend/` after an approved source-write freeze:

```sh
go run ./cmd/cloudflare-migrate snapshot-postgres-0018 \
  -schema public \
  -out /absolute/private/postgresql-0018.jsonl

go run ./cmd/cloudflare-migrate export-0018 \
  -source-jsonl /absolute/private/postgresql-0018.jsonl \
  -out /absolute/private/source-0018.json
```

`snapshot-postgres-0018` fails if a mapped table's exact canonical legacy column set differs, if a blocked/unknown table is non-empty, or if a source value cannot be represented without loss. `export-0018` rechecks stream ordering, completeness, row/table bounds, source fingerprints, digests, relationships, and the complete 0001–0018 target manifest.

## 2. Bind the reviewed bundle to its source and build the plan

```sh
go run ./cmd/cloudflare-migrate plan-0018 \
  -source-jsonl /absolute/private/postgresql-0018.jsonl \
  -bundle /absolute/private/source-0018.json \
  -canonical-bundle /absolute/private/canonical-0018.json \
  -sql-plan /absolute/private/d1-restore-0018.sql \
  -validation-sql /absolute/private/d1-validate-0018.sql
```
...
The generated SQL verifies the installed 0001–0018 schema, operational-table preconditions, exact source-backed rows, exact table counts, foreign keys, per-row/per-chunk provenance, and the canonical bundle digest. Replaying the identical SQL is idempotent. A different existing row, a non-pristine first-import runtime table, or a mismatched bundle fails closed.
...
## 4. Upgrade an already canonical 0001–0017 restore bundle

If a reviewed 0017 restore bundle has already been accepted, it can be upgraded to the 0018 canonical profile without rereading PostgreSQL:

```sh
go run ./cmd/cloudflare-migrate upgrade-0018 \
  -bundle /absolute/private/canonical-0017.json \
  -out /absolute/private/upgraded-0018.json \
  -sql-plan /absolute/private/upgraded-d1-restore-0018.sql \
  -validation-sql /absolute/private/upgraded-d1-validate-0018.sql
```

The upgrade changes the restore profile to `legacy-postgresql-to-d1-0018/v1`, appends the pinned `0018_auth_sessions.sql` SHA-256 `f39e75c2351cb3535e951034d4e85af167b68d33d3f5eabde49c6e78f39168e4`, and records the four auth-session tables as pristine `empty-before-import` target-only state. It does not read Redis, synthesize refresh sessions, or copy token material. As with `plan-0018`, the three outputs are published atomically or removed before the command fails.

## 5. Legacy 0017 commands

The `snapshot-postgres-0017`, `export-0017`, `plan-0017`, and `upgrade-0017` commands remain available for reproducing already reviewed 0017 artifacts. They do not assert or initialize the 0018 auth-session schema and should not be used for a new canonical 0018 restore.
...
The tests fingerprint all 18 canonical migration files, apply them to local SQLite, account for every canonical persistent table or explicit target-only operational table, execute and replay a restore transaction, inject schema/version/fingerprint/corruption/operational-state failures, and verify failed multi-output publication leaves no partial accepted result. Local tests are not staging or production verification.

## 7. Operator-owned remote procedure

After a separate staging database has canonical migrations 0001–0018 applied, use `remote-plan` to print repository-pinned Wrangler argv. It still does not execute them:
