...
0013_oauth_refresh_runtime.sql is intentionally unwired. It adds D1 authority for account credential versions and encrypted-envelope fingerprints, live/terminal refresh attempts, immutable audit, opaque invalidation outbox, and immutable commit witnesses. No route, scheduler, management write, Queue publisher, provider adapter, shared entry point, Wrangler configuration, generated type, or shared test setup is changed here.

The Durable Object has one narrow job: metadata-only single-flight fencing for one account. It receives only canonical account id, credential version, operation id, owner, fence, and trusted-Worker timestamps. It must never receive an envelope, token, provider response, email address, proxy detail, or another secret. It persists its first account id and rejects all future cross-account RPCs, including a caller that used the wrong object name.
...
## Future wiring obligations

- Add the OAUTH_REFRESH_AUTHORITY binding/export. The shared workerd setup already loads canonical 0013; bound DO RPC remains an integration gate.
...
- Provider adapters must apply the order above, reject PAT/non-refreshable accounts, and retain no credential material in logs or durable metadata.
...
The focused suite uses the shared setup's actual 0013 migration and deterministic lease-core tests. It does not claim a real bound-DO RPC test until shared binding wiring is authorized.
