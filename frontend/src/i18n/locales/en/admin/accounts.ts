export default {
    accounts: {
      ...
      pleaseSelectAtLeastOneGroup: 'Please select at least one active OpenAI standard group',
      invalidCloudflareBaseUrl: 'Enter a valid HTTPS upstream Base URL without credentials, query, or fragment',
      invalidCloudflareSchedulingValues: 'Concurrency and priority are outside the supported range',
      cloudflareNativeAccountHint: 'Cloudflare-native mode currently accepts OpenAI-compatible API Key accounts. Unsupported traditional account options are hidden.',
      cloudflareNativeAccountEditHint: 'Cloudflare-native mode exposes only durable account fields. Leave the API Key empty to keep the encrypted credential unchanged.',
      cloudflareNativeCredentialReplaceHint: 'The Base URL is replaced only when a new API Key is entered.',
      cloudflareNativeUnsupportedAccount: 'This account type is not migrated in Cloudflare-native mode',
