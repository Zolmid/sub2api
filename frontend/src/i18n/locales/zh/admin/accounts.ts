export default {
    accounts: {
      ...
      pleaseSelectAtLeastOneGroup: '请至少选择一个启用的 OpenAI 标准分组',
      invalidCloudflareBaseUrl: '请输入不含凭据、查询参数或片段的有效 HTTPS 上游 Base URL',
      invalidCloudflareSchedulingValues: '并发数或优先级超出支持范围',
      cloudflareNativeAccountHint: 'Cloudflare 原生模式当前接收 OpenAI 兼容 API Key 账户；未迁移的传统账户选项已隐藏。',
      cloudflareNativeAccountEditHint: 'Cloudflare 原生模式仅显示已持久化迁移的账户字段；API Key 留空会保留现有加密凭据。',
      cloudflareNativeCredentialReplaceHint: '只有输入新的 API Key 时才会同时替换 Base URL。',
      cloudflareNativeUnsupportedAccount: '此账户类型尚未迁移到 Cloudflare 原生模式',
